...
export class SmartPromptEditor {
  ...
  createInlinePrefixDom(prefix){
    ...
    const iconKey = iconName.split(/[-_\s]+/).filter(Boolean)
      .map(part => part.charAt(0).toUpperCase() + part.slice(1)).join('');
    const iconData = window.lucide?.icons?.[iconKey] || window.lucide?.icons?.Blocks;
    const icon = iconData && window.lucide?.createElement
  ...
  syncSelectionCaret(){
    ...
    const geometry = besidePrefix
      ? this.inlinePrefixCaretGeometry(prefix)
      : this.caretOverlayGeometry(selection.from);
    if(!geometry){
      this.clearSelectionCaret();
      return;
    }
  ...
  prepareInitialPointerPlacement(event){
    ...
    let hit = null;
    try { hit = this.view.posAtCoords({left:event.clientX, top:event.clientY}); }
    catch(_) { return; }
  ...
  inlinePrefixCaretGeometry(prefix){
    ...
    const caretHeight = Math.max(12, fontSize + 1) * Math.max(.1, scaleY);
    ...
    const transparentCaret = !computedCaretColor || computedCaretColor === 'auto'
      || computedCaretColor === 'transparent' || computedCaretColor === 'rgba(0, 0, 0, 0)';
    return {
      left:(prefixRect.right + marginRight * scaleX - contentLeft) / Math.max(.1, scaleX) + this.host.scrollLeft,
      top:(caretTop - contentTop) / Math.max(.1, scaleY) + this.host.scrollTop,
      height:caretHeight / Math.max(.1, scaleY),
  ...
  getSelectionRange(){
    const {from, to, anchor, head, empty} = this.view.state.selection;
  ...
  replaceText(text, options={}){
    ...
    if(options.focus !== false){ this.initialPointerPlacementPending = false; this.view.focus(); }
  ...
  referenceDragTargetAtCoords(clientX, clientY){
    ...
    // Position 1 has two DOM sides when the non-document prefix decoration is
    // present. Always use the real prompt-content side so the drop caret does
    // not jump between the chip edge and the text start as the pointer moves.
    const geometry = prefix
      ? this.inlinePrefixCaretGeometry(prefix)
      : this.caretOverlayGeometry(position, line, clientX);
