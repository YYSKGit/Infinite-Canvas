(() => {
  ...
  // frontend/prompt-geometry.mjs
  function plainCaretCoords(rect) {
    if (!rect) return null;
  ...
  // frontend/prompt-model.mjs
  var PROMPT_CLIPBOARD_MIME = "application/x-infinite-canvas-prompt+json";
  ...
  function createReferenceId(ref = {}, usedIds = /* @__PURE__ */ new Set()) {
    const preferred = String(ref.refId || ref.ref_id || "").trim();
    const base2 = preferred || `ref_${stableHash(referenceIdentity(ref))}`;
    let id = base2;
    let suffix = 2;
    while (usedIds.has(id)) id = `${base2}_${suffix++}`;
    return id;
  }
  function normalizeReference(ref = {}, usedIds = /* @__PURE__ */ new Set()) {
    const kind = normalizeReferenceKind(ref.kind);
    const normalized = {
      ...ref,
  ...
  function referenceLabel(ref, orderedReferences = []) {
    const targetIdentity = referenceIdentity(ref);
    const counters = { image: 0, video: 0, audio: 0 };
    let fallback = "";
    for (const item of orderedReferences) {
      const kind2 = normalizeReferenceKind(item.kind);
      counters[kind2] += 1;
      const label = kind2 === "video" ? `\u89C6\u9891${counters[kind2]}` : kind2 === "audio" ? `\u97F3\u9891${counters[kind2]}` : `\u56FE${counters[kind2]}`;
  ...
  function referencePlaceholder(ref, orderedReferences = []) {
    const targetIdentity = referenceIdentity(ref);
    const counters = { image: 0, video: 0, audio: 0 };
    for (const item of orderedReferences) {
      const kind2 = normalizeReferenceKind(item.kind);
      ...
      const type2 = kind2 === "video" ? "Video" : kind2 === "audio" ? "Audio" : "Image";
      return `{{${type2} ${counters[kind2]}}}`;
    }
    const kind = normalizeReferenceKind(ref.kind);
    const type = kind === "video" ? "Video" : kind === "audio" ? "Audio" : "Image";
    return `{{${type} 1}}`;
  }
  function emptyPromptDocument() {
  ...
  function textToPromptDocument(text, orderedReferences = []) {
  ...
  function promptDocumentParts(docJson, references = []) {
  ...
  function promptDocumentExchangeText(docJson, references = [], orderedReferences = references) {
    return promptDocumentParts(docJson, references).map((part) => {
      if (part.type === "text") return part.text || "";
      return referencePlaceholder(part, orderedReferences);
    }).join("");
  }
  function usedPromptReferenceIds(docJson) {
    const ids = /* @__PURE__ */ new Set();
    const walk = (node) => {
  ...
  function sliceReferenceIds(slice2) {
    const ids = /* @__PURE__ */ new Set();
  ...
  var SmartPromptEditor = class {
    ...
    createState(docJson) {
      let doc3;
      try {
        doc3 = schema.nodeFromJSON(docJson || emptyPromptDocument());
      } catch (_) {
        doc3 = schema.nodeFromJSON(emptyPromptDocument());
      }
    ...
    caretOverlayGeometry(position, lineHint = null, xHint = null) {
      let coords;
      try {
        coords = plainCaretCoords(this.view.coordsAtPos(position));
      } catch (_) {
        return null;
      }
      ...
      let anchorCoords = lineHint ? { ...coords, top: lineHint.top, bottom: lineHint.bottom } : coords;
        ...
        if ($position.parent.isTextblock) {
          ...
          if (!hasTextNeighbor && (beforeMedia || afterMedia)) {
            ...
            if (beforeBoundary && afterBoundary) {
              const overlap = Math.min(beforeBoundary.bottom, afterBoundary.bottom) - Math.max(beforeBoundary.top, afterBoundary.top);
              if (overlap > 0) {
                anchorCoords = {
          ...
          $position.parent.forEach((child, offset) => {
            ...
            [[start, 1], [end, -1]].forEach(([candidatePosition, side]) => {
              let candidate;
              try {
                candidate = this.view.coordsAtPos(candidatePosition, side);
              } catch (_) {
                return;
              }
              ...
              if (centerDistance > Math.max(2, height * 0.65)) return;
              const score = centerDistance * 1e4 + Math.abs(candidatePosition - position);
              if (score < bestScore) {
                bestScore = score;
      ...
      const fallbackHeight = (fontSize + 2) * Math.max(0.1, scaleY);
      ...
      return {
        left: (anchorCoords.left - contentLeft) / Math.max(0.1, scaleX) + this.host.scrollLeft,
        top: (caretTop - contentTop) / Math.max(0.1, scaleY) + this.host.scrollTop,
        height: Math.max(12, caretHeight / Math.max(0.1, scaleY)),
    ...
    getJSON() {
      return cloneJson(this.view.state.doc.toJSON());
    }
    getReferences() {
      return cloneJson(this.references);
    }
    getParts() {
      return promptDocumentParts(this.getJSON(), this.references);
    }
    getExchangeText() {
    ...
    setText(text, options = {}) {
      ...
      const doc3 = textToPromptDocument(String(text || ""), ordered);
      ...
      this.setValue(doc3, mergeReferenceLists(this.references, promoted), options);
    }
    clear(options = {}) {
    ...
    ensureReference(raw) {
      const identity = referenceIdentity(raw);
      const existing = this.references.find((ref2) => referenceIdentity(ref2) === identity) || this.referenceContext.find((ref2) => referenceIdentity(ref2) === identity);
      if (existing) {
        const promptOwned = this.references.find((ref2) => String(ref2.refId) === String(existing.refId));
        if (promptOwned) return promptOwned;
        const imported = normalizeReference({ ...existing, ...raw, refId: existing.refId }, new Set(this.references.map((ref2) => ref2.refId)));
        this.references = mergeReferenceLists(this.references, [imported]);
        return imported;
      }
      const used = new Set(this.references.map((ref2) => ref2.refId));
    ...
    insertReference(raw) {
      ...
      const addRightSpace = !insertion.$to.nodeAfter || inlineNeighborNeedsSpace(insertion.$to.nodeAfter, "right");
    ...
    beginPointerReferenceDrag(dom, getPos, event) {
      const from2 = typeof getPos === "function" ? getPos() : -1;
      if (!Number.isFinite(from2) || from2 < 0) return;
      this.view.focus();
      ...
      const finish = () => {
        ...
        if (moved) {
          if (Number.isFinite(targetPos)) this.moveReferenceToPosition(from2, targetPos);
          else this.scheduleSelectionCaretSync();
        } else this.scheduleSelectionCaretSync();
      };
      const cancel = () => {
    ...
    renderedPromptLines() {
      ...
      rects.sort((a, b) => a.top - b.top || a.left - b.left).forEach((rect) => {
        ...
        let line = lines.find((candidate) => {
          ...
          return Math.abs(center - candidateCenter) <= Math.max(3, Math.min(height, candidateHeight) * 0.55);
        });
        if (!line) {
          line = { ...rect };
          lines.push(line);
    ...
    renderedPromptLineAtPoint(clientX, clientY) {
      ...
      const scaleX = this.host.offsetWidth ? hostRect.width / this.host.offsetWidth : 1;
      const scaleY = this.host.offsetHeight ? hostRect.height / this.host.offsetHeight : scaleX;
      const verticalPadding = Math.max(2, 2 * Math.max(0.1, scaleY));
      const leadingPadding = Math.max(6, 6 * Math.max(0.1, scaleX));
      const candidates = this.renderedPromptLines().filter((line) => clientY >= line.top - verticalPadding && clientY <= line.bottom + verticalPadding && clientX >= line.left - leadingPadding && clientX <= line.right + Math.max(18, (line.bottom - line.top) * 0.8));
    ...
    referenceDragTargetAtCoords(clientX, clientY) {
      ...
      const horizontalTolerance = inTrailingZone ? Math.max(18, (line.bottom - line.top) * 0.8) : Math.max(10, 9 * Math.max(0.1, geometry.scaleX));
      const verticalTolerance = Math.max(4, 5 * Math.max(0.1, geometry.scaleY));
    ...
    inlineInsertionPosition(doc3, position) {
      const clamped = Math.max(0, Math.min(position, doc3.content.size));
      const $position = doc3.resolve(clamped);
    ...
    moveReferenceToPosition(from2, targetPos) {
      const view = this.view;
      const node = view.state.doc.nodeAt(from2);
      ...
      if (to === from2 || to === from2 + node.nodeSize) return false;
      const transaction = view.state.tr.delete(from2, from2 + node.nodeSize);
      if (to > from2) to -= node.nodeSize;
    ...
    renderReferenceToken(dom, refId) {
      ...
      const ref = displayReferences.find((item) => String(item.refId || "") === String(refId)) || this.references.find((item) => String(item.refId || "") === String(refId));
    ...
    handleCopyCut(view, event, cut) {
      ...
      const ids = sliceReferenceIds(slice2);
      const references = this.references.filter((ref) => ids.has(String(ref.refId || "")));
      const text = this.sliceExchangeText(slice2);
      event.clipboardData.setData(PROMPT_CLIPBOARD_MIME, JSON.stringify({ version: 1, slice: slice2.toJSON(), references }));
    ...
    importReferenceBundle(references = []) {
      ...
      references.forEach((raw) => {
        ...
        const existing = this.references.find((ref2) => referenceIdentity(ref2) === identity) || this.referenceContext.find((ref2) => referenceIdentity(ref2) === identity);
        if (existing) {
          let imported = this.references.find((ref2) => referenceIdentity(ref2) === identity);
          if (!imported) {
        ...
        const collides = requestedId && this.references.some((ref2) => String(ref2.refId) === requestedId && referenceIdentity(ref2) !== identity);
    ...
    handlePaste(view, event) {
      ...
      if (structured) {
        try {
          ...
          const slice2 = Slice.maxOpen(importedSlice.content);
          event.preventDefault();
          view.dispatch(view.state.tr.replaceSelection(slice2).scrollIntoView());
          return true;
        } catch (_) {
        }
      }
      ...
      if (text !== "") {
        ...
        const doc3 = schema.nodeFromJSON(parsed);
        const slice2 = Slice.maxOpen(doc3.content);
        event.preventDefault();
        view.dispatch(view.state.tr.replaceSelection(slice2).scrollIntoView());
        return true;
      }
    ...
    static textDocument(text, references = []) {
      return textToPromptDocument(text, references);
    }
    static partsFromJSON(docJson, references = []) {
      return promptDocumentParts(docJson || emptyPromptDocument(), references);
    }
    static exchangeText(docJson, references = [], orderedReferences = references) {
      return promptDocumentExchangeText(docJson || emptyPromptDocument(), references, orderedReferences);
    }
  };
